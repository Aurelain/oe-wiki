...
`,Le=vt;var ue,Ne,mt={info:"\u2139\uFE0F",warning:"\u26A0\uFE0F",error:"\u26D4"};function bt(e,...t){let r=e.endsWith("!")?e.startsWith("!")?"error":"warning":"info";_t(r,e,t)}function $e(e){ue=e,Ne=ue.querySelector("table")}function _t(e,t,r){let s=new Date().toLocaleTimeString("en-GB",{hour12:!1,hour:"2-digit",minute:"2-digit",second:"2-digit",fractionalSecondDigits:3}),i=document.createElement("tr");De(i,s),De(i,mt[e]),De(i,wt(t,r)),Ne.appendChild(i),ue.scrollTo({top:ue.scrollHeight,behavior:"smooth"})}function De(e,t){let r=document.createElement("td");t=t instanceof Node?t:document.createTextNode(t),r.appendChild(t),e.appendChild(r)}function wt(e,t){if(e=e.replace(/^!/,""),console.log(e,...t),!t.length)return document.createTextNode(e);let r=document.createElement("div"),n=document.createElement("div");n.innerHTML=e,n.onclick=function(){let i=this.nextElementSibling.style.display!=="none";this.nextElementSibling.style.display=i?"none":"block"},r.appendChild(n);let s=document.createElement("textarea");return s.style.display="none",s.innerHTML=Dt(t),r.appendChild(s),r}function Dt(e){let t=[];for(let r of e)typeof r=="object"&&r?r instanceof Error?t.push(r.stack):t.push(JSON.stringify(r,null,4)):t.push(r);return t.join(`
`)}var D=bt;var St=U`
    html, body {
        padding: 0;
        margin: 0;
...
`,Oe=yt;var Be=window.location.host||"localhost",te="store";async function L(e){if(!(await indexedDB.databases()).find(n=>n.name===Be))return;let r=await He();if(!r.objectStoreNames.contains(te)){r.close();return}return new Promise((n,s)=>{let i=r.transaction(te).objectStore(te).get(e);i.onsuccess=()=>{n(i.result),r.close()},i.onerror=()=>{s(i.error),r.close()}})}async function pe(e,t){let r=await He();return new Promise((n,s)=>{let i=r.transaction(te,"readwrite");i.objectStore(te).put(t,e),i.oncomplete=()=>{n(),r.close()},i.onerror=()=>{s(i.error),r.close()}})}function He(){return new Promise((e,t)=>{let r=indexedDB.open(Be);r.onupgradeneeded=n=>n.target.result.createObjectStore(te),r.onsuccess=()=>e(r.result),r.onerror=()=>t(r.error)})}async function At(e,t,r){return new Promise(n=>{let s=i=>{let a=i.data&&typeof i.data=="object"?i.data:{};a.type===t&&(e.removeEventListener("message",s),n(a.payload))};e.addEventListener("message",s),e.postMessage({type:t,payload:r})})}var Se=At;function Tt(e,t,r){e.postMessage({type:t,payload:r})}var Ge=Tt;async function Mt(e,t,r,n){return Ue(e,"",t,r,n)}async function Ue(e,t,r,n,s){t+=t?"/":"";let i={},a=await It(e);for(let[o,l]of a){let f=t+o;if(!(n&&f.match(n))){if(l.kind==="directory"){let h=await Ue(l,f,r,n,s);if(h instanceof File)return h;Object.assign(i,h)}else if(f.match(r)){let h=await l.getFile();if(s)return h;i[f]=h}}}return i}async function It(e){let t=[];try{for await(let r of e.entries())t.push(r)}catch{}return t}var je=Mt;async function kt(e){return await e?.queryPermission()==="granted"}var $=kt;function Rt(e){return document.querySelector("."+e)}var G=Rt;function Ft(e,t,r){let n;typeof t=="string"?(t.match(/^[a-z]/)&&(t="."+t),n=Array.from(document.querySelectorAll(t))):Array.isArray(t)?n=t:n=[t];for(let s of n)s.addEventListener(e,r)}var C=Ft;var de=ut(We(),1),Pt=new de.default;function Lt(e,t){let r=Ve(e),n=Ve(t),s=Pt.diff_main(r,n),i=[],a=[];for(let[h,u]of s)switch(h){case de.default.DIFF_DELETE:i.push("<del>"+u+"</del>"),a.push("<del></del>");break;case de.default.DIFF_INSERT:i.push("<ins></ins>"),a.push("<ins>"+u+"</ins>");break;default:i.push(u),a.push(u)}let o=i.join("").replaceAll("</del><ins></ins>","</del>"),l=a.join("").replaceAll("<del></del><ins>","<ins>"),f=l.split("<").length-1;return{text1:o,text2:l,differences:f/2}}function Ve(e){return e.split("&").join("&amp;").split("<").join("&lt;").split(">").join("&gt;")}var ze=Lt;var Nt={[se]:"new",[Y]:"identical",[x]:"changed"};function $t(e,t,r){let n=[];for(let s in t){let i=t[s],a=e[s];if(a!==void 0){let{text1:l,text2:f}=ze(a,i);a=l,i=f}else i=Ot(i);let o=Ct(s,a,i);n.push(...o)}r.innerHTML=n.join(""),C("click","."+H,Bt),Ht(r)}function Ct(e,t,r){let n=t===void 0?se:t===r?Y:x,s=`<div class="${fe}">${Nt[n]}</div>`,i=n===Y?ee:"",a=[];return a.push(`<div class="${H} ${n}">${s}${e}</div>`),a.push(`<div class="${K} ${i}">`),n===x&&a.push(`<pre>${t}</pre>`),a.push(`<pre>${r}</pre>`),a.push("</div>"),a}function Ot(e){return e.split("&").join("&amp;").split("<").join("&lt;").split(">").join("&gt;")}function Bt(e){e.currentTarget.nextElementSibling.classList.toggle(ee)}function Ht(e){let t=e.querySelectorAll("."+H);for(let r of t)if(!r.nextElementSibling.classList.contains(ee)){r.scrollIntoView({behavior:"smooth",block:"start"});break}}var ve=$t;var Gt=new RegExp("[ :]");function Ut(e){if(e.match(Gt)){D("Path contains an illegal character!",e);return}if(e.match("/.*?/")){D("Path contains too many slashes!",e);return}let t=e;return t=t.replace(/^Main\//,""),t=t.replace("/",":"),t=t.replaceAll("~","/"),t=t.replaceAll("_"," "),t=t.replaceAll("#",":"),t=t.replace(/\.lua$/,""),t=t.replace(/\.wiki$/,""),t}var me=Ut;var jt=new Set(["Main","Data","Media","Special","Talk","User","User talk","Project","Project talk","File","File talk","MediaWiki","MediaWiki talk","Template","Template talk","Help","Help talk","Category","Category talk","Module","Module talk"]),Wt=new Set(["css"]),Vt=new RegExp("~");function zt(e){e.match(Vt)&&D("Title contains an illegal character!",e);let{namespace:t,titleWithoutNamespace:r}=Kt(e),n=r;return n=n.replaceAll("/","~"),n=n.replaceAll(" ","_"),n=n.replaceAll(":","#"),n=Jt(n),n=Zt(n,t),t.replace(" ","_")+"/"+n}function Kt(e){let[,t,r]=e.match(/(.*?):(.*)/)||[null,"",e];return jt.has(t)||(t="",r=e),t=t||"Main",{namespace:t,titleWithoutNamespace:r}}function Jt(e){let t=(e.match(/\.([a-zA-Z]+)$/)||[null,""])[1].toLowerCase();return t==="url"?e.replace(/\.url$/,""):Wt.has(t)?e:e+".wiki"}function Zt(e,t){return t!=="Module"||e.endsWith("~doc.wiki")||e.endsWith(".css")?e:e.replace(".wiki",".lua")}var Ke=zt;function qt(e,...t){if(typeof e=="function")try{let r=e(...t);return r&&typeof r.then=="function"?r.then(n=>[n,null]).catch(n=>[null,n]):[r,null]}catch(r){return[null,r]}return e&&typeof e.then=="function"?e.then(r=>[r,null]).catch(r=>[null,r]):[e,null]}var ae=qt;var Ee;async function oe(e){if(!Ee){console.log("Please call applySettings() prior to asking!");return}let t={method:Qt(e)},r=new URL(Ee.API_URL),n=Xt(e);t.method==="GET"?r.search=n.toString():t.body=n;let[s,i]=await ae(fetch(r,t));if(!s){D("Failed to fetch!",i);return}let[a,o]=await ae(s.json());if(!a){D("Invalid json!",o);return}return a}function Je(e){let{API_URL:t}=e;Ee={API_URL:t}}function Qt(e){return!e.method||e.method==="GET"?"GET":"POST"}function Xt(e){let t={...e,format:"json",formatversion:2},{method:r}=t;if(delete t.method,r==="FORM"){let n=new FormData;for(let s in t){let i=t[s];Array.isArray(i)?n.append(s,i[0],i[1]):n.append(s,i)}return n}return new URLSearchParams(t)}var Ze=50;async function Yt(e){let t=Object.keys(e).map(s=>me(s)),r=await xt(t);if(!r)return;let n={};for(let s in r){let i=Ke(s);i&&(r[s]===void 0?D("Invalid page content!",s):n[i]=r[s])}return D(`Retrieved ${Object.keys(n).length} pages.`),n}async function xt(e){let t={};for(let r=0;r<e.length;r+=Ze){let n=Math.min(r+Ze,e.length),s=e.slice(r,n);D(`Retrieving ${r+1}-${n} of ${e.length} pages (${s[0]})...`);let i=await oe({action:"query",prop:"revisions",rvprop:"content",rvslots:"main",titles:s.join("|"),origin:"*"});if(!i)continue;let a=i.query?.pages||[];for(let o of a)o.missing||(t[o.title]=o.revisions?.[0]?.slots?.main?.content)}return t}var qe=Yt;function er(e,t){e.classList.toggle(F,t===F),e.classList.toggle(N,t===N),e.classList.toggle(B,t===B)}var W=er;async function tr(e,t){let r=rr(e,t);if(!r.length)return D("All pages are already updated."),!0;let n=await nr();if(!n)return D("Cannot get csrf token!"),!1;let s=0;for(let{title:i,content:a}of r)await ir(i,a,n)&&s++;return D(`Successfully saved ${s} pages.`),!0}async function rr(e,t){let r=[];t=t||{};for(let n in e){let s=e[n],i=t[n];if(s!==i){let a=me(n);r.push({title:a,content:s})}}return r}async function nr(){return(await oe({action:"query",meta:"tokens"}))?.query?.tokens?.csrftoken}async function ir(e,t,r){return D(`Saving content for "${e}"...`),(await oe({method:"POST",action:"edit",title:e,text:t,token:r}))?.edit?.result!=="Success"?(D("Save failed!",e,t),!1):!0}var Qe=tr;function sr(){return document.location.href.replace(/[^\/]*$/,"")+"api.php"}var Xe=sr;var Ye,xe,ye,be,_e,we,J,V={parserResult:null,mirrorResult:null,progressingButton:null,hasDirAccess:!1,hasPreviewed:!1,hasSaved:!1};async function ar(e,t){Ye=e,xe=t,ye=G(j),be=G(q),_e=G(Q),we=G(X),C("click",ye,lr),C("click",be,cr),C("click",_e,fr),C("click",we,hr),J=await L(z),Z({hasDirAccess:await $(J)}),Je({API_URL:Xe()})}function Z(e){Object.assign(V,e),or()}function or(){let{hasDirAccess:e,progressingButton:t,mirrorResult:r,hasPreviewed:n,hasSaved:s}=V;Ye.classList.toggle(le,!!t),W(ye,e?F:N);let i;t===q?i=B:i=r?F:null,W(be,i),be.disabled=!e;let a;t===Q?a=B:a=n?F:null,W(_e,a),_e.disabled=!e;let o;t===X?o=B:o=s?F:null,W(we,o),we.disabled=!e}async function lr(){let{hasDirAccess:e}=V,t=!1;if(J?e?t=!0:await J.requestPermission():t=!0,t){let[r]=await ae(window.showDirectoryPicker,{mode:"read"});J=r,await pe(z,J)}Z({parserResult:null,mirrorResult:null,progressingButton:null,hasDirAccess:await $(J),hasPreviewed:!1,hasSaved:!1})}async function cr(){await Ae(q),Z({mirrorResult:await qe(V.parserResult)})}async function fr(){await Ae(Q);let e=window.open("","Preview");if(!e)return;let t=e.document;t.open(),t.write(`
        <!DOCTYPE html>
        <html lang="en">
        <head>
            <title>Preview</title>
